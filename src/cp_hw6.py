...
def computeIntrinsic(images, checkerboard, dW):
    ...
    for fname in images:
        ...
        #img = cv2.resize(img, (int(w/n_), int(h/n_)))
...
def computeExtrinsic(img_path, mtx, dist, dX, dY):
    ...
    #color_img = cv2.resize(color_img, (int(w/n_), int(h/n_)))
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.plot([0, dX, dX, 0, 0], [0, 0, dY, dY, 0], color = 'red')
    C = np.matmul(-rmat.T, tvec)
    ax.scatter(C[0], C[1], C[2], s=10, marker="s", color = 'green')
    ax.quiver(C[0], C[1], C[2], corner_rays[0,:], corner_rays[1,:], corner_rays[2,:], color='yellow')
